...
def solve_maze(
    ...
    x_in, y_in = first_exit[0], first_exit[1]
    x_out, y_out = second_exit[0], second_exit[1]
    if encircled_exit(grid, first_exit) or encircled_exit(grid, second_exit):
        return grid, None

    grid_copy = deepcopy(grid)
    rows, cols = len(grid), len(grid[0])
    grid_copy[x_in][y_in] = 1
    for i in range(rows):
        for j in range(cols):
            if grid_copy[i][j] in (" ", "X"):
                grid_copy[i][j] = 0

    k = 1
    while grid_copy[x_out][y_out] == 0:
        ns_grid = make_step(grid_copy, k)
        if ns_grid == grid_copy:
            return grid, None
        grid_copy = ns_grid
        k += 1
    path = shortest_path(grid_copy, second_exit)
